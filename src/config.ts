...
function parseScript(script: string): {scriptType: string; input: string[]; output: string[]} {
  ...
  const cleanInput = [
    ...new Set(inputMatch.split(',').map((ext) => `.${ext}`.replace(/^\./, '').trim())),
  ];
  let cleanOutput: string[] = [];
  if (outputMatch) {
    cleanOutput = [
      ...new Set(outputMatch.split(',').map((ext) => `.${ext}`.replace(/^\./, '').trim())),
    ];
    ...
    cleanOutput = Array.from(
      new Set(cleanInput.map((ext) => srcFileExtensionMapping[ext] || [ext])),
    );
  }
...
export function createBuildPipeline(plugins: SnowpackPlugin[]): Record<string, SnowpackPlugin[]> {
  ...
  plugins.forEach((plugin) => {
    plugin.input.forEach((ext) => {
      if (!pipeline[ext]) {
        pipeline[ext] = [];
      }
      pipeline[ext].push(plugin);
    });
  });
...
function normalizeConfig(config: SnowpackConfig): SnowpackConfig {
  ...
  // If any plugins defined knownEntrypointss, add them here
